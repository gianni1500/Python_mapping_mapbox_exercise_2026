# =============================================================================
# SCRIPT 2 — Section 930: Geocode Winery Addresses & Create a Basic Map
# =============================================================================
# WHAT THIS SCRIPT DOES:
#   1. Reads the winery data CSV created by Script 1
#   2. For each winery, sends the address to the Mapbox Geocoding API
...
#   You must have run Script 1 first so that data/napa_wineries.csv exists.
#
# OUTPUT:
#   data/napa_wineries_geocoded.csv   — winery data with lat/lon columns added
#   data/napa_wineries_map_basic.html — interactive map (open in browser)
#
# TO RUN:
...
# here, then call it multiple times (once per winery) in our loop below.
#
...
# =============================================================================
# STEP 4: READ THE WINERY DATA CSV
# =============================================================================
print("=" * 60)
print("  Napa Valley Winery Geocoder & Basic Map Builder")
print("=" * 60)

# Define the path to our input file (output of Script 1)
input_path = os.path.join("data", "napa_wineries.csv")

# Check that the file actually exists before trying to open it
if not os.path.exists(input_path):
    print(f"\nERROR: Input file not found: {input_path}")
    print("Please run Script 1 first (01_scrape_napa_wineries.py)")
    exit()
...
print(f"\nSuccessfully geocoded: {geocoded_count} out of {len(df)} wineries")
print(f"Could not geocode:      {len(df) - geocoded_count} wineries")
...
geocoded_path = os.path.join("data", "napa_wineries_geocoded.csv")
...
# --- Filter to Only Geocoded Wineries ---
# We can only place markers for wineries where we have coordinates.
...
# We center the map on the average lat/lon of all our mapped wineries.
# This ensures the map opens centered on the Napa Valley.
...
# =============================================================================
map_output_path = os.path.join("data", "napa_wineries_map_basic.html")
