...
#   Uses the geocoded winery data and swaps out the plain OpenStreetMap
...
#   4. Script 2 must have been run so data/napa_wineries_geocoded.csv exists
#
# OUTPUT:
#   data/napa_wineries_map_custom.html
#
# TO RUN:
...
print("=" * 60)
print("  Napa Valley Wineries — Custom Mapbox Style Map")
print("=" * 60)
...
# =============================================================================
input_path = os.path.join("data", "napa_wineries_geocoded.csv")
...
# Calculate the center of all mapped wineries
...
<div style="position: fixed; top: 10px; left: 50%; transform: translateX(-50%);
    ...
    🍷 Napa Valley AVA Wineries — Custom Style
</div>
"""
...
for i, row in df_mapped.iterrows():
    ...
    phone_display = row["Phone"] if pd.notna(row["Phone"]) and row["Phone"] else "Not listed"
    address_display = (
        row["Full_Address"]
...
output_path = os.path.join("data", "napa_wineries_map_custom.html")

napa_map.save(output_path)
...
print(f"  data/napa_wineries_map_basic.html   ← plain OpenStreetMap")
print(f"  data/napa_wineries_map_custom.html  ← YOUR custom Mapbox style")
